...
<script>
  window.addEventListener('DOMContentLoaded', function () {
    const formWrapper = document.getElementById('ee-form-6');
    const desktopTarget = document.getElementById('form-desktop');
    const mobileTarget = document.getElementById('form-mobile');

    function relocateForm() {
      if (window.innerWidth <= 768) {
        mobileTarget.style.display = 'block';
        desktopTarget.style.display = 'none';
        if (!mobileTarget.contains(formWrapper)) {
          mobileTarget.appendChild(formWrapper);
        }
      } else {
        mobileTarget.style.display = 'none';
        desktopTarget.style.display = 'block';
        if (!desktopTarget.contains(formWrapper)) {
          desktopTarget.appendChild(formWrapper);
        }
      }
    }

    relocateForm(); // On load
